...
public final class XxeScanner extends AbstractScanner {
    ...
    /**
     * Substitute path parameters with default values.
     */
    private String substitutePathParameters(ApiEndpoint endpoint) {
        String path = endpoint.getPath();

        // Replace all path parameters with default values
        for (ParameterSpec param : endpoint.getPathParameters()) {
            String defaultValue = getDefaultValueForParameter(param);
            path = path.replaceAll("\\{" + param.getName() + "\\}", defaultValue);
        }
    ...
    /**
     * Get a default value for a parameter based on its name and type.
     */
    private String getDefaultValueForParameter(ParameterSpec param) {
    ...
    private TestRequest createXmlRequest(
        ...
        String path = substitutePathParameters(endpoint);
    ...
    private TestRequest createRequestWithXmlParameter(
        ...
        try {
            // URL-encode the XML content for query parameter
            String encodedXml = java.net.URLEncoder.encode(xmlContent, "UTF-8");
            String url = context.buildUrl(path) + "?" + paramName + "=" + encodedXml;
            ...
            String url = context.buildUrl(path) + "?" + paramName + "=" + xmlContent;
            return TestRequest.builder()
                .url(url)
                .method(endpoint.getMethod())
                .headers(context.getAuthHeaders())
