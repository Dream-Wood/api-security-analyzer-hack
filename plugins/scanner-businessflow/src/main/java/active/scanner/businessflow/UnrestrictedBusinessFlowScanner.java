...
public final class UnrestrictedBusinessFlowScanner extends AbstractScanner {
    ...
    // Expanded to cover banking, fintech, and general business operations
    private static final List<String> SENSITIVE_OPERATIONS = List.of(
        // Financial operations
        "purchase", "buy", "order", "payment", "transfer", "withdraw", "deposit",
        "transaction", "payout", "refund", "charge",

        // Account management
        "create", "register", "signup", "close", "cancel", "suspend", "activate",

        // Authorization and consent (banking/fintech specific)
        "consent", "authorize", "approve", "agreement", "grant",

        // Data operations
        "delete", "remove", "reset", "update", "modify",

        // Verification and validation
        "verify", "confirm", "validate", "authenticate",

        // User interactions
        "vote", "comment", "post", "submit", "send",

        // Business actions
        "apply", "request", "claim", "redeem", "execute"
    );
     ...
     * Enhanced for banking/fintech APIs with consent and agreement operations.
     */
    private Severity determineSeverityByOperation(ApiEndpoint endpoint) {
        String path = endpoint.getPath().toLowerCase();
        String method = endpoint.getMethod().toUpperCase();

        // Critical operations - financial, deletion, authorization
        if (path.contains("payment") || path.contains("purchase") || path.contains("buy") ||
            path.contains("transfer") || path.contains("withdraw") || path.contains("deposit") ||
            path.contains("payout") || path.contains("refund") || path.contains("charge") ||
            path.contains("delete") || path.contains("remove") || path.contains("close") ||
            path.contains("reset") || path.contains("cancel") ||
            // Banking/fintech specific - consent and agreement operations are critical
            path.contains("consent") || path.contains("agreement") || path.contains("authorize") ||
            method.equals("DELETE")) {
            return Severity.CRITICAL;
        }

        // High severity - account operations, data modification, verification
        if (path.contains("register") || path.contains("signup") || path.contains("create") ||
            ...
            path.contains("apply") || path.contains("request") || path.contains("grant") ||
            path.contains("activate") || path.contains("suspend") ||
