...
public final class BolaScanner implements VulnerabilityScanner {
     ...
     * Now supports both numeric IDs and composite IDs (e.g., team200-1, acc-1001).
     */
    private BolaTestResult testIdEnumeration(
        ...
        // Test Case 1: Simple numeric IDs
        List<String> numericTestIds = List.of("1", "2", "3", "4", "5", "10", "100", "999");

        for (String testId : numericTestIds) {
            String url = buildUrlWithId(endpoint, testId, context);
        ...
        // Test Case 2: Composite IDs with common prefixes (banking/fintech style)
        // Patterns: team200-1, acc-1001, usr-123, client-456, pmt-789
        List<String> compositePrefixes = List.of("team200-", "team100-", "acc-", "usr-", "client-", "user-", "pmt-", "agr-", "consent-");
        List<String> compositeSuffixes = List.of("1", "2", "3", "100", "1001");

        for (String prefix : compositePrefixes) {
            for (String suffix : compositeSuffixes) {
                String compositeId = prefix + suffix;
                String url = buildUrlWithId(endpoint, compositeId, context);
                ...
                if (response.getStatusCode() == 200) {
                    successfulAccesses++;
                    accessibleIds.add(compositeId);

                    // If we found a working composite ID, stop testing that prefix
                    // and move to the next one to save requests
                    break;
                }
            }
        }

        // Test Case 3: UUID variants (common in some APIs)
        List<String> uuidVariants = List.of(
            "00000000-0000-0000-0000-000000000001",
            "00000000-0000-0000-0000-000000000002",
            "11111111-1111-1111-1111-111111111111"
        );

        for (String uuid : uuidVariants) {
            String url = buildUrlWithId(endpoint, uuid, context);
            ...
            if (response.getStatusCode() == 200) {
                successfulAccesses++;
                accessibleIds.add(uuid);
            }
        }

        // If we can access multiple IDs (3 or more), it's likely a BOLA vulnerability
        if (successfulAccesses >= 3) {
            ...
            VulnerabilityReport vulnerability = VulnerabilityReport.builder()
                ...
                .description(
                    ...
                    "Successfully accessed " + successfulAccesses + " different resources by testing various ID formats " +
                    "(numeric, composite, UUID). This allows attackers to access other users' data by guessing or " +
                    "enumerating IDs across different organizational styles and ID conventions."
                )
                ...
                .addRecommendation("Use non-sequential, non-guessable UUIDs instead of incremental or predictable IDs")
                .addRecommendation("Return 403 Forbidden for unauthorized access attempts, not 404")
                .addRecommendation("Avoid exposing ID formats that reveal organizational structure (e.g., team200-1)")
                .reproductionSteps(
                    "1. Authenticate as a user\n" +
                    "2. Access " + url + " with IDs: " + accessibleIds + "\n" +
                    "3. Observe that all IDs are accessible without ownership verification\n" +
                    "4. ID formats tested: numeric, composite (prefix-number), UUID"
                )
                .build();
    ...
    private String buildUrlWithId(ApiEndpoint endpoint, String idValue, ScanContext context) {
        ...
        // Safety check: use default value if idValue is null
        if (idValue == null) {
            idValue = "1";
        }
